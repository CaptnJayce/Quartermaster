...
assistant_msg = {
    'role': 'system',
    'content': (
        f'You are an AI assistant that has another AI model working to get you live data from search '
        f'engine results that will be attached before a USER PROMPT. The current date and time is {current_time} '
        f'({time_zone}). You must analyze the SEARCH RESULT and use any relevant data to generate the most useful & '
        f'intelligent response an AI assistant that always impresses the user would generate.'
    )
}
...
contains_data_msg = (
    ...
    'Consider the USER_PROMPT that was sent to the actual AI assistant and whether the PAGE_TEXT contains the data '
    'needed to construct an intelligent, correct response. This web PAGE_TEXT was retrieved from a search engine '
    'using the SEARCH_QUERY that is also attached to user messages in this conversation. All user messages in this '
    'conversation will have the format of: \n'
    ...
    'PAGE_TEXT does indeed contain the reliable data for the AI assistant to use as context to respond. Respond '
